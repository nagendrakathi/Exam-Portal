...
export default function QuestionCard({ q, index, onSelect }) {
  return (
    <div className="bg-white p-4 rounded shadow-sm">
      <div className="mb-3 font-medium">Q{index + 1}. {q.prompt}</div>
        ...
        {q.options.map((opt, i) => {
          ...
          return (
            <button key={i}
              ...
              className={`option-btn text-left p-3 rounded border ${selected ? "bg-teal-50 border-teal-300" : "hover:bg-slate-50"}`}
            >
